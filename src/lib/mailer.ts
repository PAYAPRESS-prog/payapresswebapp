...
export function adminSignupNotificationEmail(info: AdminSignupInfo): { subject: string; html: string; text: string } {
  ...
  const row = (k: string, v: string) =>
    `<div style="padding:10px 14px;margin:0 0 8px;background:#faf7f2;border:1px solid #f0e6d8;border-radius:8px;">
       <div style="font-size:11px;color:#9a7b4f;text-transform:uppercase;letter-spacing:0.04em;font-weight:700;">${k}</div>
       <div style="font-size:14px;color:#16181d;font-weight:600;word-break:break-word;line-height:1.5;margin-top:2px;">${v}</div>
     </div>`;
  const html = baseLayout(`
